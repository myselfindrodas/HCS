...
class ShiftchangeRequestAllListViewModel(private val mainRepository: MainRepository) : ViewModel() {

    fun shiftchangeallList(authtoken: String) = liveData(Dispatchers.IO) {
        emit(Resource.loading(data = null))

        try {
            emit(Resource.success(data = mainRepository.myshiftchangerequestlist(authtoken)))
        } catch (e: Exception) {
