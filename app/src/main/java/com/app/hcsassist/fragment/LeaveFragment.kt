...
class LeaveFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        fragmentLeaveBinding.btnAllleave.setOnClickListener {
...
//        fragmentLeaveBinding.recTypesleaves.setAdapter(typesOfLeavesAdapter)
    ...
    fun changetitle(text:String){
        ...
        fragmentLeaveBinding.allLeaveInfoTxt.text = text

    }

    fun allleavebuttoncolorchanges(){

            fragmentLeaveBinding.btnAllleave.setBackgroundResource(R.drawable.grey_box)
            fragmentLeaveBinding.tvshortcode.setTextColor(ContextCompat.getColor(mainActivity, R.color.black))
    }
                    ...
                    it?.let { resource ->
                        when (resource.status) {
                            Status.SUCCESS -> {
                                ...
                                for (j in it.data.leaveTaken!!){
                                    val leaveModel = LeaveModel()
                                    leaveModel.count = j?.count
                                    leaveModel.short_code = j?.short_code
                                    leaveModel.no_of_leave = j?.no_of_leave
                                    leaveModel.leave_type = j?.leave_type
